...
class Call:
    ...
    def setClass(self, className):
        if className != 'GLOBAL':
            self.scope = className+'::'+self.scope
...
#trakcs the imported modules -> used when calling a function from a different module
class Import:
...
class Walker:
    ...
    def walkItem(self, path, key, item):
        if isinstance(item, list): # []
            ...
            if Type == "ClassDef": #class definitions
                self.classDefs.append((item.get('name'), item))
            if Type == 'Import': #import statements
                for i in item.get('names'):
                    self.imports.append(Import(i.get('name')))
            if Type == 'ImportFrom': #import statements
                self.imports.append(Import(item.get('module')))
                for i in item.get('names'):
                    self.imports[LAST_ELEMENT].setFuncs(i.get('name'))
        else:
            pass
...
class Parser:
    ...
    def parse(self, filename):
        for n, call in enumerate(self.calls):
            ...
            self.final[LAST_ELEMENT].setScope(self.getScope(filename, call)) #set scope of Call object
            self.final[LAST_ELEMENT].setClass(self.getClass(call)) #set class of Call object
            self.final[LAST_ELEMENT].setArgs(self.getArgs(call, True, name)) #set arguments of Call object
    ...
    def getName(self, call):
        ...
        if name == None:
            ...
            else: #check to see function is imported
                 name = self.checkImports(name, call.get('func').get('value').get('id', None))
        else:
            name = self.checkImports(name)
        return name

    #chekc to see if function is imported from another module
    def checkImports(self, call, attr='1'):
        ...
        if attr == '1':
            for i in self.walker.imports:
                for f in i.funcs:
                    if call == f:
                        name = i.name+'.'+call #add name of module to function call name
        else:
            name = 'OBJ.'+call #OBJ. will be at the begnining of every member function, and an imported function is usually a member function
            for i in self.walker.imports:
                if attr == i.name:
                    name = i.name+'.'+call #add name of module to function call name
        return name
    ...
    def getSimpleValue(self, item):
        ...
        if retval:
            ...
            if retval == None: #not int/double or variable
                retval = item.get('s', None) #if not None, it's a string
        return retval
    ...
    def parseArgs(self, item):
        ...
        if item.get("_type", None) == 'Call': #function call as argument of another call
            ...
            for n, c in enumerate(self.calls):
                if str(item) == str(c): #add arguments inside call name
            ...
            if argName != None: #literal argument (int, string, char...)
                ...
                if Type == 'Tuple': #(a, b) -> (a|b) (don't want commas in CSV)
                    for e in item['elts']:
                        argName = argName+self.parseArgs(e)+'|'
                    argName = '('+argName[:LAST_ELEMENT]+')'
                if Type == 'List': #[1, 2, 3] -> [1|2|3]
                    for e in item['elts']:
                        argName = argName+self.parseArgs(e)+'|'
                    argName = '['+argName[:LAST_ELEMENT]+']'
                if Type == 'Dict': #{key: value}
                    ...
                    argName = '{'+argName[:LAST_ELEMENT]+'}' #{k1:v1|k2:v2}
                ...
                if Type == 'Lambda': #lambda k: print(k)
                    ...
                    argName = 'lambda('+'|'.join(a)+')' #lambda(arg1|arg2)
    ...
    def getSubscript(self, item):
        #parses subscript by where the : is located (different keywords in AST)
        def parseSub(item):
            global error
            a = ':'
        ...
        #get the name of the subscript argument
    ...
    def getArgs(self, c, call, callName):
        args = []
        if c['args'] != []: #normal arguments for both lambda and call
            for a in c['args']:
                args.append(self.parseArgs(a))
        if call: #arguments specific to a call
    ...
    #get's number of Parameters for a function
    ...
    def prepareFuncs(self, filename):
        functions = []
        for func in self.walker.functionDefs:
            className = self.getClass(func[1])
            if className != 'GLOBAL':
                func[0] = className+'::'+func[0]
                className = 'MEMBER'
            functions.append([filename, className, func[0], self.getParamNum(func[1])])
        return functions

    #prints out all functions in the program to the CSV file
    def printFuncs(self, f, funcs):
        writer = csv.writer(f)
...
def main(astfile, pyfile, csvfile, funcfile):
    ...
    #get the className of the call by comparing to member functions in the program's functions list
    #this might be problematic if two classes have a function with the same name
    functions = parser.prepareFuncs(pyfile)
    for e, f in enumerate(parser.final):
        if f.className == 'MEMBER': #member function in call list
            for funcs in functions:
                if funcs[1] == 'MEMBER': #member function in function list
                    #f.name[4:] gets rid of OBJ. and funcs[2].split("::")[1] gets the name of the function without the class name
                    if f.name[4:] == funcs[2].split('::')[1]:
                        #append class name to call name -> funcs[2].split('::')[0] gets the name of the class without the function name
                        parser.final[e].name = funcs[2].split('::')[0]+"::"+f.name[4:]
